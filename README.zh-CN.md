...
<p align="center">
  ...
  把项目、会话、知识芯片、Skills 和自动化集中到一个本地优先的个人智能工作台。
</p>
...
Harness Workbench 不是另一套聊天内核。它保留 DSH 原生会话作为唯一真相源，在外层增加项目管理、知识芯片、本地 RAG、分层 Skill 管理、会话文件仓、待办、定时 Agent、每日总结和 Codex 接入，最终把零散的 AI 对话整理成一个可以持续使用的个人工作系统。
...
| --- | --- | --- | --- |
| **项目会话** | 围绕一个 Workspace 持续工作 | 项目目录、关联知识芯片、全局与项目 Skills、显式 `@` 引用 | 待办、会话文件、每日总结、定时任务、知识芯片、Skills 和项目会话历史 |
| **知识芯片会话** | 围绕一组文档研究和问答 | 当前知识芯片的文档与向量检索结果、全局 Skills、显式 `@` 引用 | 会话文件、文档来源、引用脚注、原始文件打开与下载 |
| **独立会话** | 临时问题、自由探索、跨项目讨论 | 全局 Skills，以及显式 `@` 的会话、知识芯片、Workspace 路径和会话文件 | 持久 File Vault、Subagent 活动和全局定时，不强制创建项目 |
...
任意会话生成 Session ID 后，右侧“会话文件”可以上传与知识芯片相同的文本、Office、HTML 和代码文件。这些文件不会生成 Embedding，也不会进入向量索引；原始文件保存在 Workbench File Vault，只有当前消息显式写入 `@文件/文件名` 时才把完整正文注入模型上下文。单轮引用正文合计上限为 32,000 个 Unicode 字符，超限或解析失败会明确阻止发送，不会静默截断或退化成向量检索。
...
1. **左侧导航**：新建会话、首页、全部会话、知识芯片、最近会话、Skills 和设置。用户熟悉的 **Skills** 入口固定位于设置上方。
2. **中间会话区**：完整保留 DSH 原生 Chat、Trajectory、模型与推理等级、实时流、工具调用、审批、附件、队列、重试和上下文统计。
3. **右侧项目工具舱**：六个图标页签使用固定顺序：**待办、会话文件、每日总结、定时任务、知识芯片、Skills**。鼠标悬浮或键盘聚焦时显示完整名称，Skills 页签直接绑定当前项目。

桌面宽屏保持三列；中等宽度把项目工具舱切换为右侧抽屉；移动端使用互斥导航与工具抽屉。六页签使用紧凑激活标记与键盘游标式焦点，同时保留 Escape 关闭、焦点恢复、`prefers-reduced-motion` 和低透明度降级。

## Skills：全局与项目的能力分层

左侧边栏的 **Skills** 入口会打开 **Skills - 技能矩阵**，在一个独立管理面中处理全局与项目两种作用域。项目会话的右侧工具舱还提供紧凑的 **Skills** 页签，只管理当前项目。Workbench 负责文件与生命周期，DSH 原生 Skill Provider 继续负责发现、加载和执行。

| 作用域 | 规范目录 | 生效范围 |
| --- | --- | --- |
| **全局** | `<DSH_HOME>/skills/<skill-name>/` | 所有项目、独立会话和知识芯片会话 |
| **项目** | `<项目目录>/.dsh/skills/<skill-name>/` | 仅当前项目内的会话；同名项目 Skill 会覆盖全局 Skill |

项目 Skill 目录在第一次成功导入时才创建。管理页同时显示已启用、已停用、无效和被覆盖状态，支持显示目录、启用、停用、删除以及同作用域的同名替换。另一作用域的同名 Skill 不算冲突。

导入只接受根目录包含 `SKILL.md` 的本地目录，或本地 ZIP。ZIP 既可以包含单个 Skill，也可以使用固定的 `skills/<skill-name>/...` 集合结构。集合包会在写入前先展示预览，确认后再以一个可恢复事务安装；校验、路径穿越、符号链接、体积限制或后续安装任一失败，都不会留下部分集合。

从 Codex Plugin 导出的 Skill 集合可以导入，但 ZIP 必须只包含固定的 `skills/` 目录树；带有该目录之外文件的原始 Plugin 包会被拒绝。Workbench 不安装 Plugin Manifest、Hook、MCP Server 或其他插件运行时能力，也不会在导入时执行包内脚本。
...
- Workspace 文件
- 当前 Session File Vault 中不向量化的会话文件
- 关联或可用的知识芯片
- 其他 Workbench 会话

图片继续走 DSH 0.1.1-rc.2 原生图片链路。普通会话文件使用 Workbench File Vault，以满足“按 Session 持久保存、后续可 `@文件` 复用”的语义，并且不会进入 `documents`、Chunk、Embedding 或 LanceDB。项目 Workspace 文件仍走 DSH 原生路径引用，知识芯片文档则保留独立的向量化流程。
...
| 项目、会话范围、待办、定时任务、总结、知识芯片元数据、本地检索和 Skill 文件生命周期 | Harness Workbench |
| Skill 发现、加载与执行 | DSH 原生 Skill Provider 与运行时 |
...
- Skill 集合导入只接受固定的纯 `skills/` 目录树，原始 Plugin 包、Hook、Manifest 和 MCP Server 不会被安装或执行。
