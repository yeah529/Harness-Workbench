...
var WORKBENCH_SESSION_SELECT = "SELECT ws.*, sch.name AS schedule_name, CASE WHEN sch.id IS NULL THEN 'chat' ELSE 'schedule' END AS session_type, CASE ws.scope_kind WHEN 'project' THEN p.name WHEN 'knowledge_base' THEN kb.name ELSE '\u72EC\u7ACB' END AS context_name FROM workbench_sessions ws LEFT JOIN projects p ON ws.scope_kind = 'project' AND p.id = ws.scope_id LEFT JOIN knowledge_bases kb ON ws.scope_kind = 'knowledge_base' AND kb.id = ws.scope_id LEFT JOIN schedules sch ON sch.session_id = ws.session_id ";
...
function createRepositories(db) {
  function orphanSessionFilesForSessions(sessionIds) {
    ...
    return [...ownership].filter(([, state]) => state.hasTarget && !state.hasOutside).map(([sha256]) => ({ sha256 }));
  }
  const projects = {
  ...
  const workbenchSessions = {
    ...
    list({ scopeKind, scopeId, lifecycleStatus = null, archived = null, limit = 100, offset = 0 }) {
      ...
      const archiveFilter = archived === true ? "AND ws.archived_at IS NOT NULL " : archived === false ? "AND ws.archived_at IS NULL " : "";
      const params = [normalizedScope.kind, normalizedScope.id];
      if (lifecycleStatus != null) params.push(lifecycleStatus);
      return db.prepare(
        WORKBENCH_SESSION_SELECT + "WHERE ws.scope_kind = ? AND ws.scope_id IS ? " + lifecycleFilter + archiveFilter + "ORDER BY ws.updated_at DESC, ws.rowid DESC LIMIT ? OFFSET ?"
    ...
    archive(sessionId, now = /* @__PURE__ */ new Date()) {
      const iso = nowIso(now);
      const info = db.prepare(
    ...
    restore(sessionId, now = /* @__PURE__ */ new Date()) {
      const iso = nowIso(now);
      const info = db.prepare(
    ...
    latest({ scopeKind, scopeId }) {
      const row = db.prepare(
        WORKBENCH_SESSION_SELECT + "WHERE ws.scope_kind = ? AND ws.scope_id IS ? AND ws.lifecycle_status = 'active' AND ws.archived_at IS NULL ORDER BY ws.updated_at DESC, ws.rowid DESC LIMIT 1"
  ...
  const sessionFiles = {
    create({
      ...
      db.prepare(
        "INSERT INTO session_files (session_id, sha256, original_name, mime_type, size, parse_status, parse_error, context_text, context_code_points, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
      ).run(
        sessionId,
        sha256,
        originalName,
  ...
  const schedules = {
    ...
    failRunning({ error = "interrupted", finishedAt = /* @__PURE__ */ new Date() } = {}) {
      const info = db.prepare(
  ...
  const maintenance = {
    purgeContainer({
      ...
      return transaction(db, () => {
        ...
        if (JSON.stringify(currentSessions) !== JSON.stringify(expectedSessions) || JSON.stringify(currentDocuments) !== JSON.stringify(expectedDocuments) || JSON.stringify(currentFiles) !== JSON.stringify(expectedFiles)) {
...
function createIndexQueue({ repos, indexer }) {
  ...
  return {
    ...
    enqueue(task) {
      ...
      const entry = { task, resolve: resolve5, promise };
    ...
    idle() {
      ...
      return new Promise((resolve5) => idleWaiters.push(resolve5));
    },
    /**
...
function stripSessionFileReferences(text, rows) {
  const input = String(text ?? "");
...
// src/host/session-files.js
var MAX_SESSION_FILE_CONTEXT_CODE_POINTS = 32e3;
var SESSION_FILE_ERROR_CODES = Object.freeze({
...
function buildFileContext(files) {
  ...
  const body = files.map(
    (file) => `  <file id="${file.id}" name="${escapeXmlAttr(file.originalName)}">
...
function createSessionFileVault({ dataDir, repos }) {
  ...
  const vaultDataDir = join5(dataDir, "session-vault");
  const objectPath = (sha256) => join5(vaultDataDir, "files", sha256);
  ...
  async function uploadOne({ sessionId, stream, originalName }) {
    ...
    try {
      ...
      contextText = parsed.sections.map((section) => `[${section.locator}] ${section.text}`).join("\n\n");
      contextCodePoints = codePointLength2(contextText);
    } catch (error) {
      parseStatus = "failed";
    ...
    try {
      ...
      if (repos.sessionFiles.countBySha256(saved.sha256) === 0) {
        await rm2(objectPath(saved.sha256), { force: true });
      }
  ...
  async function remove(id) {
    ...
    if (repos.sessionFiles.countBySha256(removed.sha256) === 0) {
      await rm2(objectPath(removed.sha256), { force: true });
    }
    return removed;
  }
...
function createContextResolver({ repos, sourceAccess } = {}) {
  ...
  function rebase({ sessionId, toScope }) {
    ...
    return resolve5({ sessionId });
  }
  function resolveForPrompt({ sessionId, oneShotSources = [] }) {
    const persistent = resolve5({ sessionId }).filter((source) => source.available);
  ...
  return { resolve: resolve5, validate, setOverride, removeOverride, rebase, resolveForPrompt };
}

// src/host/skill-package.js
import { lstat, mkdir as mkdir2, mkdtemp, rename as rename2, rm as rm3, writeFile } from "node:fs/promises";
import { dirname, isAbsolute, join as join6, parse, posix, resolve as resolve2, sep } from "node:path";
import { Inflate } from "fflate";
import { load as loadYaml } from "js-yaml";
var SKILL_PACKAGE_LIMITS = Object.freeze({
  archiveBytes: 50 * 1024 * 1024,
  entries: 1e3,
  expandedBytes: 100 * 1024 * 1024,
  singleFileBytes: 50 * 1024 * 1024
});
var SKILL_ERROR_CODES = Object.freeze({
...
var UNIX_SYMLINK_MODE = 40960;
var UNIX_FILE_TYPE_MASK = 61440;
var UNIX_REGULAR_MODE = 32768;
var UNIX_DIRECTORY_MODE = 16384;
...
function inspectZipCentralDirectory(archiveBytes) {
  ...
  if (entryCount === 65535 || centralSize === ZIP64_SENTINEL || centralOffset === ZIP64_SENTINEL) {
  ...
  for (let offset2 = centralEnd; offset2 + 4 <= endOffset; offset2 += 1) {
    const signature = readUint32(view, offset2);
  ...
  for (let index = 0; index < entryCount; index += 1) {
    ...
    if (extras.some(([id]) => id === 1)) throw unsafeArchive("ZIP64 entries are not supported", { index });
...
function inflateBounded(compressed, limit) {
  ...
  if (compressed.byteLength === 0) {
    ...
    for (let offset2 = 0; offset2 < compressed.byteLength; offset2 += chunkSize) {
      const end = Math.min(offset2 + chunkSize, compressed.byteLength);
      stream.push(compressed.subarray(offset2, end), end === compressed.byteLength);
    }
  }
...
function extractZipEntry(bytes, entry, maxOutput, archiveEnd) {
  ...
  if (localOffset + 30 > bytes.length || readUint32(view, localOffset) !== 67324752) {
...
async function assertDestinationPathSafe(destination) {
  ...
  const absolute = resolve2(destination);
...
function normalizeEntryName(name, directory = false) {
  ...
  const normalized = directory && normalizedName.endsWith("/") ? normalizedName.slice(0, -1) : normalizedName;
  ...
  if (parts2.length === 0 || parts2.some((part) => part.length === 0 || part === "." || part === "..")) {
    throw unsafeArchive("ZIP entry path is unsafe", { name });
  }
  return posix.join(...parts2);
}
...
function packageLayout(normalizedEntries, { allowCollection }) {
  ...
  if (skillEntries.length === 1) {
    ...
    let wrapper2 = "";
    if (skillParts.length === 2) wrapper2 = skillParts[0];
    else if (skillParts.length !== 1) throw packageError("ZIP archive contains a nested wrapper");
    const entries2 = normalizedEntries.map((entry) => {
      if (wrapper2 && entry.directory && entry.path === wrapper2) return { ...entry, path: "" };
      if (wrapper2 && !entry.path.startsWith(`${wrapper2}/`)) throw packageError("ZIP archive contains multiple wrappers");
      return { ...entry, path: wrapper2 ? entry.path.slice(wrapper2.length + 1) : entry.path };
    });
    if (!entries2.some((entry) => entry.path === "SKILL.md")) throw packageError("ZIP archive wrapper is invalid");
    return { kind: "skill", wrapper: wrapper2, entries: entries2, bundles: [{ folder: null, prefix: "", skillPath: "SKILL.md" }] };
  }
  if (!allowCollection) throw packageError("ZIP archive must contain exactly one SKILL.md");
  const parts2 = skillEntries.map((entry) => entry.path.split("/"));
  const direct = parts2.every((value) => value.length === 3 && value[0] === "skills" && value[2] === "SKILL.md");
  const wrapper = direct ? "" : parts2.every((value) => value.length === 4 && value[1] === "skills" && value[3] === "SKILL.md" && value[0] === parts2[0][0]) ? parts2[0][0] : null;
  ...
  const folders = new Set(parts2.map((value) => value[direct ? 1 : 2]));
...
async function extractSkillPackageInternal({ archiveBytes, destination, limits, allowCollection }) {
  ...
  const skills = layout.bundles.map((bundle) => {
    ...
    const files = [...dataByPath.keys()].filter((path) => path.startsWith(bundle.prefix)).map((path) => path.slice(bundle.prefix.length)).sort();
    return {
      ...identity,
      files,
  ...
  try {
    ...
    staging = await mkdtemp(join6(parent, ".cpwb-skill-package-"));
    for (const [index, bundle] of layout.bundles.entries()) {
      for (const file of skills[index].files) {
        const target = layout.kind === "collection" ? join6(staging, skills[index].name, ...file.split("/")) : join6(staging, ...file.split("/"));
        await mkdir2(dirname(target), { recursive: true });
        await writeFile(target, dataByPath.get(`${bundle.prefix}${file}`), { flag: "wx" });
      }
    }
    await assertDestinationPathSafe(absoluteDestination);
    await rename2(staging, absoluteDestination);
    staging = void 0;
  } catch (error) {
    if (staging) await rm3(staging, { recursive: true, force: true }).catch(() => {
    });
    if (error instanceof SkillManagerError) throw error;
...
async function extractSkillPackage(options) {
...
var DEFAULT_AUTOMATION_PROMPTS = Object.freeze({
  ...
  todoPrompt: [
    "\u8BF7\u6839\u636E\u9879\u76EE {{projectId}} \u5728 {{date}} \u7684\u9879\u76EE\u4F1A\u8BDD\u6B63\u6587\u548C\u65B0\u589E\u77E5\u8BC6\u5185\u5BB9\uFF0C\u5224\u65AD {{nextDate}} \u662F\u5426\u6709\u660E\u786E\u9700\u8981\u5904\u7406\u7684\u5F85\u529E\u3002",
    "\u4E0D\u8981\u628A\u5DF2\u6709\u5F85\u529E\u3001\u5DE5\u5177\u8C03\u7528\u3001\u5206\u6790\u8FC7\u7A0B\u6216\u6A21\u578B\u601D\u8003\u6539\u5199\u6210\u65B0\u5F85\u529E\uFF1B\u4E0D\u8981\u81C6\u9020\u6CA1\u6709\u4E8B\u5B9E\u4F9D\u636E\u7684\u4E8B\u9879\u3002",
    "\u4EE5\u4E0B\u6570\u636E\u662F\u672C\u6B21\u751F\u6210\u7684\u5168\u90E8\u8F93\u5165\uFF1B\u4E0D\u8981\u8BFB\u53D6\u5DE5\u4F5C\u533A\uFF0C\u4E0D\u8981\u8C03\u7528\u4EFB\u4F55\u5DE5\u5177\u3002",
    "\u6709\u660E\u786E\u5F85\u529E\u65F6\u53EA\u8F93\u51FA\u4EE5\u201C- \u201D\u5F00\u5934\u7684\u9010\u884C\u6E05\u5355\uFF1B\u6CA1\u6709\u660E\u786E\u5F85\u529E\u65F6\u53EA\u8F93\u51FA NONE\uFF0C\u4E0D\u8981\u8F93\u51FA\u6807\u9898\u6216\u89E3\u91CA\u3002"
  ].join("\n")
});
...
function todoDedupKey(title) {
  return normalizeTodoTitle(title).replace(/\s*[（(]\d{4}-\d{1,2}-\d{1,2}(?:[ T]\d{1,2}:\d{2})?[）)]\s*$/, "").toLowerCase();
}
...
async function makeTodoPrompt(repos, projectId, date, nextDate, timeZone, template, projectConversations) {
  ...
  const conversations = await projectConversations({ projectId, date, timeZone });
  const hasConversationContent = conversations.some((conversation) => conversation?.messages?.some((message) => normalizeTodoTitle(message?.text) !== ""));
  if (!hasConversationContent && knowledgeChanges.length === 0) return null;
  const openTodoTitles = todos.filter((todo) => todo.done !== true).map((todo) => todo.title).filter(Boolean);
  return [
...
function createScheduler({ repos, clock = () => /* @__PURE__ */ new Date(), runPrompt, intervalMs = 6e4, timeZone = DEFAULT_TIME_ZONE, automationPrompts = DEFAULT_AUTOMATION_PROMPTS, projectConversations = async () => [] } = {}) {
  ...
  async function runAutoTodos(project, now, dueDate, zone = getTimeZone()) {
    ...
    try {
      ...
      const existingTitles = new Set((repos.todos?.list?.({ projectId: project.id, timeZone: zone }) ?? []).filter((todo) => todo.done !== true).map((todo) => todoDedupKey(todo.title)));
...
var SKILL_NAME_PATTERN2 = /^[a-z0-9]+(?:-[a-z0-9]+)*$/;
...
function safeSkillSummary(value) {
  ...
  if (Array.isArray(value.files) && value.files.length <= SKILL_PACKAGE_LIMITS.entries && value.files.every((file) => typeof file === "string" && file.length > 0 && file.length <= 1024 && !file.includes("\0") && !file.startsWith("/") && !/^[A-Za-z]:[\\/]/.test(file) && !file.split(/[\\/]/).some((part) => part === "" || part === "." || part === ".."))) {
...
function safeSkillErrorDetails(error) {
  ...
  if (error.code === SKILL_ERROR_CODES.CONFLICT) {
    ...
    return existing || incoming ? { ...existing ? { existing } : {}, ...incoming ? { incoming } : {} } : void 0;
  }
  if (error.code === SKILL_ERROR_CODES.COLLECTION_CONFIRMATION_REQUIRED && details.kind === "collection" && Number.isSafeInteger(details.count) && details.count >= 2 && Number.isSafeInteger(details.conflictCount) && details.conflictCount >= 0 && Array.isArray(details.skills) && details.skills.length === details.count && details.skills.length <= SKILL_PACKAGE_LIMITS.entries) {
    const skills = [];
    for (const value of details.skills) {
      const summary = safeSkillSummary(value);
      if (!summary || !SKILL_NAME_PATTERN2.test(summary.name ?? "") || typeof value.conflict !== "boolean") return void 0;
      const existing = value.existing === void 0 ? void 0 : safeSkillSummary(value.existing);
      if (value.conflict && (!existing || existing.name !== summary.name)) return void 0;
      skills.push({ ...summary, conflict: value.conflict, ...existing ? { existing } : {} });
    }
    if (skills.filter((skill) => skill.conflict).length !== details.conflictCount || new Set(skills.map((skill) => skill.name)).size !== details.count) return void 0;
    const sourceName = typeof details.sourceName === "string" && details.sourceName.length <= 1024 && !/[\0\r\n]/.test(details.sourceName) ? details.sourceName : "";
  ...
  if (typeof details.name === "string" && SKILL_NAME_PATTERN2.test(details.name)) safe.name = details.name;
...
function toApiError(err) {
  ...
  if (err instanceof SkillManagerError) {
    ...
    if (status === void 0) return new ApiError(500, "INTERNAL_ERROR", "internal server error");
...
async function readRawBody(req, limit = SKILL_PACKAGE_LIMITS.archiveBytes) {
  ...
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    const onData = (chunk) => {
      ...
      if (total > limit) {
        fail2(new ApiError(413, SKILL_ERROR_CODES.ARCHIVE_TOO_LARGE, "Skill ZIP exceeds the byte limit"), true);
        return;
      }
      chunks.push(buf);
    };
    ...
    const onError = (error) => fail2(error);
    const onAborted = () => fail2(new Error("request body was aborted"));
    req.on("data", onData);
    req.once("end", onEnd);
    req.once("error", onError);
...
function createApi({ repos, queue, ollama, retriever, dataDir, services = {}, sessions, sessionFiles, settings, embeddingFactory, onEmbeddingConfigChange, credentials, codexAuth, dshAdapter, skills, logger = console, networkEnv = process.env }) {
  ...
  const hasSessions = typeof sessions?.materializeDraft === "function" && typeof sessions?.confirmDraft === "function" && typeof sessions?.renameSession === "function" && typeof sessions?.moveSession === "function" && typeof sessions?.deleteSession === "function";
  ...
  const hasSessionArchive = typeof sessions?.archiveSession === "function" && typeof sessions?.restoreSession === "function";
  ...
  function parseSkillScope(scope, projectId) {
    ...
    if (scope === "global") {
      if (projectId !== void 0) throw new ApiError(422, "INVALID_FIELD", "projectId is only valid for project scope");
      return { scope };
    }
  ...
  function parseSkillQuery(url) {
    const allowed = /* @__PURE__ */ new Set(["scope", "projectId"]);
    ...
    const projectId = projectIdRaw == null ? void 0 : queryPositiveInt(projectIdRaw, "projectId");
  ...
  function parseSkillJsonScope(body, fields) {
    ...
    if (projectId !== void 0 && !isPositiveInt(projectId)) {
      throw new ApiError(422, "INVALID_FIELD", "projectId must be a positive integer");
    }
    return parseSkillScope(scope, projectId);
  }
  function parseSkillName(raw) {
    if (typeof raw !== "string" || !SKILL_NAME_PATTERN2.test(raw)) {
  ...
  function parseSkillImportHeaders(req) {
    try {
      ...
      if (replaceRaw !== void 0 && replaceRaw !== "true" && replaceRaw !== "false") {
      ...
      if (confirmCollectionRaw !== void 0 && confirmCollectionRaw !== "true" && confirmCollectionRaw !== "false") {
  ...
  function publicMaintenanceJob(job) {
    ...
    for (const key of [
      ...
      if (job[key] !== void 0) output[key] = job[key];
    }
  ...
  async function handleSessionFileContent(req, res, { params, url }) {
    ...
    let bytes;
    try {
      bytes = await readFile2(sessionFileVault.contentPath(file));
    } catch {
      throw new SessionFileError(SESSION_FILE_ERROR_CODES.NOT_FOUND, "session file bytes are unavailable: " + id);
    }
    res.writeHead(200, {
  ...
  async function handleDocumentContent(req, res, { params, url }) {
    ...
    let bytes;
    try {
      bytes = await readFile2(join7(dataDir, "files", document.sha256));
    } catch {
  ...
  async function handleReindex(req, res, { params }) {
    ...
    queue.enqueue({
      documentId: id,
      filePath: join7(dataDir, "files", doc.sha256),
  ...
  async function handleProjectDeletionPlan(req, res, { params }) {
    const id = parseId(params.id);
    const extended = maintenance?.containerPlan ? await maintenance.containerPlan("project", id) : null;
    const plan = extended ? {
  ...
  async function handleProjectDelete(req, res, { params, url }) {
    ...
    const plan = typeof services.deleteProject === "function" ? await services.deleteProject({ projectId: id, sessionPolicy }) : (() => {
  ...
  async function handleKnowledgeBasesList(req, res) {
    ok(res, repos.knowledgeBases.list().map((knowledgeBase) => {
      ...
      const overview = {
        ...
        state: documents.length === 0 ? "empty" : hasAttention ? "attention" : readyFileCount === documents.length ? "ready" : "indexing",
        latestIndexedAt
      };
      const recentDocuments = [...documents].sort((a, b) => String(b.indexedAt || b.createdAt).localeCompare(String(a.indexedAt || a.createdAt))).slice(0, 3).map(({ id, originalName, mimeType, size, status, createdAt, indexedAt, error }) => ({
        id,
        originalName,
        mimeType,
        size,
  ...
  async function handleKnowledgeBaseDeletionPlan(req, res, { params }) {
    const id = parseId(params.id);
    const extended = maintenance?.containerPlan ? await maintenance.containerPlan("knowledge_base", id) : null;
    const plan = extended ? {
  ...
  async function handleKnowledgeBaseDelete(req, res, { params, url }) {
    ...
    const plan = typeof services.deleteKnowledgeBase === "function" ? await services.deleteKnowledgeBase({ knowledgeBaseId: id, sessionPolicy }) : (() => {
      for (const sessionId of preview.sessionIds) {
        if (sessionPolicy === "delete") repos.workbenchSessions.remove(sessionId);
        else repos.workbenchSessions.updateScope({ sessionId, scope: { kind: "independent", id: null } });
      }
  ...
  async function handleIndexReindex(req, res) {
    const queued = repos.documents.list().map((doc) => {
      repos.documents.updateIndexState(doc.id, { status: "parsing", error: null, indexedAt: null });
      queue.enqueue({ documentId: doc.id, filePath: join7(dataDir, "files", doc.sha256), originalName: doc.originalName, mimeType: doc.mimeType });
      return doc.id;
    });
  ...
  async function handleChatSessionCreate(req, res) {
    ...
    const body = await readJsonBody(req);
    const allowedFields = /* @__PURE__ */ new Set(["scope", "title", "pinnedSources"]);
  ...
  async function handleChatSessionPatch(req, res, { params }) {
    ...
    if (body.operation === "archive" || body.operation === "restore") {
      ...
      ok(res, body.operation === "archive" ? await sessions.archiveSession(sessionId) : await sessions.restoreSession(sessionId));
      return;
    }
  ...
  async function dispatch(req, res, url) {
    ...
    if (maintenance?.isLocked?.() === true && !["GET", "HEAD", "OPTIONS"].includes(method)) {
      throw new ApiError(
        503,
...
function isWorkbenchFileContext(message) {
  return message?.source?.kind === "plugin" && message.source.plugin === "dsh-cyberpunk-workbench" && message.source.form === "file-context";
}
...
function createWorkbenchRagPreStep({ retriever, scope, onQuestion, contextResolver, sessionId, fileContext }) {
...
function escapeCitationBody(value) {
  return escapeXmlText2(value).replace(/\[source/gi, "&#91;source").replace(/\[\/source\]/gi, "&#91;/source&#93;");
}
...
function assembleKnowledgePrompt(citations, { question = "", maxCodePoints = MAX_CONTEXT_CODE_POINTS } = {}) {
  ...
  for (const citation of citations) {
    ...
    const sourceLine = '[source id="' + escapeXmlAttr2(citation.sourceId ?? "") + '" document-id="' + (Number.isSafeInteger(documentId) && documentId > 0 ? String(documentId) : "") + '" file="' + escapeXmlAttr2(citation.originalName ?? "") + '" locator="' + escapeXmlAttr2(citation.locator ?? "") + '"]\n';
...
function createSessionService({
  ...
  function registerHandle(sessionId, { agent = null, dispose: dispose2, cleanup, scope, owned }) {
  ...
  async function reopenScopedSession({ sessionId }) {
    ...
    if (ctx.agents.get(SessionId(sessionId)) !== void 0) {
      ...
      registerHandle(sessionId, { agent: liveAgent2, dispose: null, cleanup: cleanup2, scope, owned: false });
  ...
  async function submitPrompt({ sessionId, question, oneShotSources = [], unattended = false }) {
    ...
    const work = async () => {
      ...
      if (unattended) {
        ...
        if (typeof tools?.get === "function" && tools.get("ask_user_question") !== void 0) {
          if (typeof tools.restrict !== "function") {
            throw new Error("unattended scheduled runs require the rc.2 scoped tools.restrict seam");
          }
          const dispose2 = tools.restrict({ deny: ["ask_user_question"] });
          if (typeof dispose2 === "function") policyDisposers.push(dispose2);
        }
        if (typeof entry.agent?.ctx?.systemPrompt?.section === "function") {
          const dispose2 = entry.agent.ctx.systemPrompt.section({
...
async function loadCodexAccessToken({ codexAuth = "disabled", codexHome, env = process.env } = {}) {
  ...
  const file = join8(codexHome || env.CODEX_HOME || defaultCodexHome(), "auth.json");
  let raw;
  try {
...
// src/host/maintenance.js
import { createHash as createHash4, randomUUID as randomUUID3 } from "node:crypto";

// src/maintenance/rc2-storage.js
import {
  access,
  copyFile,
  cp,
...
import { dirname as dirname2, isAbsolute as isAbsolute2, join as join9, relative, resolve as resolve3 } from "node:path";
var SESSION_ID = /^[A-Za-z0-9._-]+$/;
var DATABASE_MEMBERS = Object.freeze([
  "workbench.sqlite",
...
function assertSessionId2(value) {
  if (typeof value !== "string" || !SESSION_ID.test(value) || value === "." || value === ".." || value.includes("..")) {
...
async function readRc2Documents(dshHome) {
  const storageRoot = join9(dshHome, "storages");
  const workspaceFile = join9(storageRoot, "workspace.json");
  const projectionFile = join9(storageRoot, "session_projcache.json");
...
function normalizeParent(value) {
  ...
  if (typeof value === "string") return assertSessionId2(value);
  if (typeof value === "object" && !Array.isArray(value)) {
    return assertSessionId2(value.sessionId);
  }
...
function parentForSession(entry) {
  ...
  ].filter((value) => value != null).map(normalizeParent);
...
async function probeRc2PurgeBackend({ dshHome, dataDir }) {
  try {
    ...
    const databasePath = resolve3(normalizedData, "workbench.sqlite");
    const delta = relative(normalizedData, databasePath);
    if (delta.startsWith("..") || isAbsolute2(delta)) {
...
async function readSessionDescendants({ dshHome, rootSessionIds }) {
  const normalizedHome = assertAbsoluteDirectoryInput(dshHome, "dshHome");
  const roots = new Set((rootSessionIds ?? []).map(assertSessionId2));
...
function recoveryCommand(env) {
  ...
  if (value && !/[\r\n]/.test(value) && !/(access[_-]?token|authorization|api[_-]?key|bearer)/i.test(value)) return value;
...
function createMaintenanceService({
  ...
  async function containerPlan(kindInput, idInput) {
    ...
    const orphanSessionFiles = [...plan.orphanSessionFiles].sort(
  ...
  async function finalizeStartupJob() {
    ...
    try {
      for (const sessionId of [
        .../* @__PURE__ */ new Set([...job.sessionIds, ...job.descendantSessionIds])
      ]) {
...
var PURGE_STATES = Object.freeze([
  "queued",
  "stopping",
  "quarantining",
  "native_refs_updated",
...
var ACTIVE_PURGE_STATES = Object.freeze(
  PURGE_STATES.filter((state) => state !== "completed" && state !== "restored")
);
var NEXT_STATES = /* @__PURE__ */ new Map([
  ["queued", /* @__PURE__ */ new Set(["stopping", "restored"])],
  ["stopping", /* @__PURE__ */ new Set(["quarantining", "restoring", "restored"])],
  ["quarantining", /* @__PURE__ */ new Set(["native_refs_updated", "restoring"])],
  ["native_refs_updated", /* @__PURE__ */ new Set(["restarting", "restoring"])],
  ["restarting", /* @__PURE__ */ new Set(["workbench_finalizing", "restoring"])],
  ["workbench_finalizing", /* @__PURE__ */ new Set(["verifying", "restoring"])],
  ["verifying", /* @__PURE__ */ new Set(["completed", "restoring"])],
  ["restoring", /* @__PURE__ */ new Set(["restored", "rollback_pending"])],
  ["rollback_pending", /* @__PURE__ */ new Set(["restoring"])]
]);
var SAFE_ID = /^[A-Za-z0-9](?:[A-Za-z0-9_-]{0,126}[A-Za-z0-9])?$/;
...
async function writeJsonAtomic(path, value) {
  const temporaryPath = `${path}.${process.pid}.${randomUUID4()}.tmp`;
  let handle;
  try {
...
function resolveMaintenanceRoot({ dshHome }) {
  if (typeof dshHome !== "string" || dshHome.trim() === "") {
    throw new TypeError("dshHome is required");
  }
  return join10(dshHome, "cyberpunk-workbench", "maintenance");
}
function createPurgeJobStore({ dshHome }) {
  const root = resolveMaintenanceRoot({ dshHome });
  const readyRoot = join10(root, "ready");
  const lockPath = join10(root, "active.lock.json");
  const jobDirectory = (jobId) => join10(root, assertSafeId(jobId, "job id"));
  const statePath = (jobId) => join10(jobDirectory(jobId), "state.json");
  const manifestPath = (jobId) => join10(jobDirectory(jobId), "manifest.json");
  const readyPath = (generation) => join10(readyRoot, `${assertSafeId(generation, "generation")}.json`);
  async function ensureRoot() {
    await mkdir4(root, { recursive: true, mode: 448 });
  }
  async function read(jobId) {
    return readJson2(statePath(jobId));
  }
  return {
    root,
    async create(request) {
      ...
      await ensureRoot();
      await mkdir4(jobDirectory(jobId), { mode: 448 });
      const created = {
        ...cloneJson(request),
        jobId,
        state: "queued",
    ...
    async listIncomplete() {
      ...
      return jobs.sort(
        (left, right) => String(left.createdAt).localeCompare(String(right.createdAt)) || left.jobId.localeCompare(right.jobId)
      );
    },
    async arm(jobId) {
...
`, "utf8");
    ...
    async releaseLock(owner) {
      const normalized = normalizeOwner(owner);
      const current = await readJson2(lockPath);
      if (current.pid !== normalized.pid || current.generation !== normalized.generation) {
...
import { basename, dirname as dirname3, isAbsolute as isAbsolute3, join as join11, relative as relative2, resolve as resolve4 } from "node:path";
var PERMISSION_CODES = /* @__PURE__ */ new Set(["EACCES", "EPERM", "EROFS"]);
var INTERNAL_NAMES = /* @__PURE__ */ new Set([".disabled", ".staging", ".transactions"]);
var defaultFileOps = Object.freeze({
  mkdir: fsMkdir,
  lstat: fsLstat,
  mkdtemp: fsMkdtemp,
  readdir: fsReaddir,
...
var SKILL_NAME_PATTERN3 = /^[a-z0-9]+(?:-[a-z0-9]+)*$/;
var TRANSACTION_ID_PATTERN = /^[A-Za-z0-9-]+$/;
var locks = /* @__PURE__ */ new Map();
var recoveryLocks = /* @__PURE__ */ new Map();
...
function serializeMany(keys, operation) {
  ...
  const acquire = (index) => index >= ordered.length ? operation() : serialize(ordered[index], () => acquire(index + 1));
  return acquire(0);
}
...
async function canonicalPathIdentity(ops, path) {
    ...
    if (stat) {
      ...
      try {
        ...
        if (error?.code === "ENOENT") {
          ...
          if (parent2 === current) return absolutePath;
          current = parent2;
          continue;
        }
        throwMapped(error, "realpath");
      }
      return resolve4(canonical, ...suffix);
    }
...
async function assertRootPathSafe(ops, rootPath, anchorPath = dirname3(rootPath), { checkInternals = true, requireDirectories = checkInternals } = {}) {
  const absoluteRoot = resolve4(rootPath);
  const absoluteAnchor = resolve4(anchorPath);
  const descendant = relative2(absoluteAnchor, absoluteRoot);
  if (descendant.startsWith("..") || isAbsolute3(descendant)) {
  ...
  for (const current2 of [
    join11(rootPath, ".disabled"),
    join11(rootPath, ".staging"),
    join11(rootPath, ".transactions")
  ]) {
    const stat = await inspectPath(current2);
    if (stat?.isSymbolicLink()) {
      throw mutationError(SKILL_ERROR_CODES.PERMISSION_DENIED, "Skill root contains a symbolic link");
    }
    if (requireDirectories && stat && !stat.isDirectory()) {
...
async function assertRecoveryPathsSafe(ops, rootPath, paths, descriptor) {
  ...
  for (const path of Object.values(paths)) {
    const pathRelative = relative2(rootPath, path);
...
function pathFor(rootPath, value) {
  return relative2(rootPath, value).split("\\").join("/");
}
function safeName(name) {
  return typeof name === "string" && SKILL_NAME_PATTERN3.test(name);
}
...
async function copyTree(ops, source, destination) {
  ...
  for (const rawEntry of await readDirectory(ops, source)) {
    ...
    const from = join11(source, name);
    const to = join11(destination, name);
...
async function writeTransaction(ops, transactionsPath, descriptor) {
  const descriptorPath = join11(transactionsPath, `${descriptor.id}.json`);
  const temporaryPath = join11(transactionsPath, `.${descriptor.id}.json.tmp`);
...
async function replaceTransaction(ops, transactionsPath, descriptor) {
  const descriptorPath = join11(transactionsPath, `${descriptor.id}.json`);
  const temporaryPath = join11(transactionsPath, `.${descriptor.id}.json.update.tmp`);
...
function transactionPaths(rootPath, descriptor) {
  ...
  if (descriptor.finalRelative !== finalRelative || descriptor.stagingRelative !== `.staging/${id}/incoming` || descriptor.previousRelative !== `.staging/${id}/previous`) return null;
  return {
    final: join11(rootPath, finalRelative),
    incoming: join11(rootPath, ".staging", id, "incoming"),
    previous: join11(rootPath, ".staging", id, "previous"),
    staging: join11(rootPath, ".staging", id)
  };
}
function collectionTransactionPaths(rootPath, descriptor) {
  ...
  for (const item of descriptor.items) {
    ...
    if (item.finalRelative !== finalRelative || item.incomingRelative !== incomingRelative || item.previousRelative !== previousRelative) return null;
    items.push({
      ...item,
      final: join11(rootPath, finalRelative),
      incoming: join11(rootPath, incomingRelative),
      previous: join11(rootPath, previousRelative),
      alternate: state === "disabled" ? join11(rootPath, item.name) : join11(rootPath, ".disabled", item.name)
    });
  }
  return { staging: join11(rootPath, ".staging", descriptor.id), items };
}
    ...
    for (const { item, final, incoming, previous } of inspected.reverse()) {
      if (item.existed) {
        if (previous) {
          ...
          await callFs("mkdir", ops.mkdir, dirname3(item.final), { recursive: true });
...
async function scanBundle({ rootPath, bundlePath, state, ops }) {
  const diagnostics = [];
  const skillPath = join11(bundlePath, "SKILL.md");
  let skillStat;
  try {
...
async function scanCatalogRoot({ rootPath, ops }) {
  ...
  for (const rawEntry of entries) {
    ...
    const path = join11(rootPath, name);
  ...
  const disabledPath = join11(rootPath, ".disabled");
  let disabledStat;
  try {
  ...
  if (disabledStat?.isSymbolicLink()) {
    ...
    for (const rawEntry of await readDirectory(ops, disabledPath)) {
      const name = entryName(rawEntry);
      if (typeof name !== "string" || name === "") continue;
      const path = join11(disabledPath, name);
      const stat = await inspect(ops, path, "lstat");
      if (stat.isSymbolicLink()) {
        diagnostics.push(diagnostic(path, "symlink", "Skill entries cannot be symbolic links"));
        continue;
  ...
  uniqueCandidates.sort((left, right) => compareStrings(left.name, right.name) || (left.state === right.state ? 0 : left.state === "enabled" ? -1 : 1));
...
async function resolveTarget({ scope, projectId, dshHome, repos, ops }) {
  ...
  if (scope === "global") {
    ...
    const rootPath2 = resolve4(dshHome, "skills");
    return {
      scope: { kind: "global" },
      rootPath: rootPath2,
      anchorPath: resolve4(dshHome),
      identity: await canonicalPathIdentity(ops, rootPath2)
    };
  }
  ...
  const rootPath = resolve4(projectPath, ".dsh", "skills");
  return {
...
async function recoverTransactions({ rootPath, ops, identity }) {
  ...
  const transactionsPath = join11(rootPath, ".transactions");
  ...
  for (const rawEntry of await readDirectory(ops, transactionsPath)) {
    ...
    const descriptorPath = join11(transactionsPath, name);
    ...
    if (!paths) throw recoveryError(descriptor);
    const alternate = descriptor.state === "disabled" ? join11(rootPath, descriptor.name) : join11(rootPath, ".disabled", descriptor.name);
    ...
    if (!finalStat && previousStat) {
      await callFs("mkdir", ops.mkdir, dirname3(paths.final), { recursive: true });
      try {
...
function createSkillManager({ dshHome, repos, fileOps = defaultFileOps, revealPath: injectedRevealPath } = {}) {
  ...
  async function readState(target, name) {
    const enabledPath = join11(target.rootPath, name);
    const disabledPath = join11(target.rootPath, ".disabled", name);
  ...
  async function importCollection({ target, packageSummary, extractedRoot, sourceName, replace, confirmCollection }) {
    ...
    return serializeMany(lockKeys, async () => {
      ...
      const stagingRoot = join11(target.rootPath, ".staging", transactionId);
      const stagingIncoming = join11(stagingRoot, "incoming");
      const transactionsPath = join11(target.rootPath, ".transactions");
      try {
        await callFs("mkdir", ops.mkdir, stagingIncoming, { recursive: true });
        for (const { summary } of records) {
          await copyTree(ops, join11(extractedRoot, summary.name), join11(stagingIncoming, summary.name));
        }
      ...
      try {
        ...
        try {
          ...
          for (const [index, record] of records.entries()) {
            ...
            const finalPath = join11(target.rootPath, item.finalRelative);
            const incomingPath = join11(target.rootPath, item.incomingRelative);
            if (record.existing) {
              const previousPath = join11(target.rootPath, item.previousRelative);
              await callFs("mkdir", ops.mkdir, dirname3(previousPath), { recursive: true });
              await callFs("rename", ops.rename, record.existing.path, previousPath);
            } else {
              await callFs("mkdir", ops.mkdir, dirname3(finalPath), { recursive: true });
            }
  ...
  async function importArchive({ scope, projectId, archiveBytes, sourceName, replace = false, confirmCollection = false } = {}) {
    ...
    const temporaryRoot = await callFs("mkdtemp", ops.mkdtemp, join11(tmpdir(), ".cpwb-skill-import-"));
    let extractedRoot;
    try {
      const canonicalTempRoot = await callFs("realpath", ops.realpath, temporaryRoot);
      extractedRoot = join11(canonicalTempRoot, "incoming");
      ...
      return await serialize(key, async () => {
        ...
        const finalPath = existing?.state === "disabled" ? join11(target.rootPath, ".disabled", summary.name) : join11(target.rootPath, summary.name);
        const transactionsPath = join11(target.rootPath, ".transactions");
        if (!existing) {
          try {
          ...
          const previous = join11(stagingRoot, "previous");
          const descriptor = {
            version: 1,
            id: transactionId,
            name: summary.name,
  ...
  async function setEnabled({ scope, projectId, name, enabled } = {}) {
    ...
    return serialize(`${target.identity}\0${name}`, async () => {
      ...
      const destination = desired === "disabled" ? join11(target.rootPath, ".disabled", name) : join11(target.rootPath, name);
      if (desired === "disabled") await callFs("mkdir", ops.mkdir, dirname3(destination), { recursive: true });
      try {
  ...
  async function list({ scope, projectId } = {}) {
    ...
    if (scope === "project") {
      const globalRoot = resolve4(dshHome, "skills");
...
function createScheduledRunPrompt(sessionService) {
  return async function runScheduledPrompt({ kind = "schedule", projectId, prompt, schedule = null }) {
    ...
    try {
      if (kind === "schedule") {
        const result2 = await sessionService.runScheduledSession({
          projectId,
        ...
        session = { sessionId: result2.sessionId };
        return result2;
      }
  ...
  ctx.effect(() => {
    ...
    try {
      ...
      const sessionWorkspace = async ({ kind, scopeId }) => {
        ...
        const path = kind === "knowledge_base" ? join12(dataDir, "knowledge-bases", String(scopeId)) : join12(dataDir, "sessions", "independent");
      ...
      const deleteContainer = async ({ kind, id, sessionPolicy }) => {
        ...
        try {
          ...
          for (const document of plan.orphanDocuments) {
            await unlink(join12(dataDir, "files", document.sha256)).catch((error) => {
              if (error?.code !== "ENOENT") throw error;
            });
          }
          return removed;
      ...
      maintenanceStartup = maintenance.isLocked() ? maintenance.finalizeStartupJob().then(startOrdinaryServices) : startOrdinaryServices();
