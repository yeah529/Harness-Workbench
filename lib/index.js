...
import { lstat, mkdir as mkdir2, mkdtemp, rename as rename2, rm as rm2, writeFile } from "node:fs/promises";
import { dirname, isAbsolute, join as join5, parse, posix, resolve as resolve2, sep } from "node:path";
...
async function extractSkillArchive({ archiveBytes, destination, limits }) {
  ...
  const declaredSizeByPath = /* @__PURE__ */ new Map();
  ...
  for (const [name, data] of actualEntries) {
    ...
    const relative3 = wrapper ? normalized.startsWith(`${wrapper}/`) ? normalized.slice(wrapper.length + 1) : null : normalized;
    if (!relative3) throw packageError("ZIP archive contains multiple wrappers");
    if (data.byteLength !== declaredSizeByPath.get(relative3)) {
      throw unsafeArchive("ZIP expanded size does not match its metadata", { name });
    }
    dataByPath.set(relative3, data);
  }
  ...
  try {
    ...
    staging = await mkdtemp(join5(parent, ".cpwb-skill-package-"));
    for (const file of files) {
      const target = join5(staging, ...file.split("/"));
...
function createApi({ repos, queue, ollama, retriever, dataDir, services = {}, sessions, settings, embeddingFactory, onEmbeddingConfigChange, credentials, codexAuth, dshAdapter, skills, logger = console, networkEnv = process.env }) {
  ...
  async function handleDocumentContent(req, res, { params, url }) {
    ...
    let bytes;
    try {
      bytes = await readFile2(join6(dataDir, "files", document.sha256));
    } catch {
  ...
  async function handleReindex(req, res, { params }) {
    ...
    queue.enqueue({
      documentId: id,
      filePath: join6(dataDir, "files", doc.sha256),
  ...
  async function handleIndexReindex(req, res) {
    const queued = repos.documents.list().map((doc) => {
      repos.documents.updateIndexState(doc.id, { status: "parsing", error: null, indexedAt: null });
      queue.enqueue({ documentId: doc.id, filePath: join6(dataDir, "files", doc.sha256), originalName: doc.originalName, mimeType: doc.mimeType });
      return doc.id;
    });
...
function assembleKnowledgePrompt(citations, { question = "", maxCodePoints = MAX_CONTEXT_CODE_POINTS } = {}) {
  ...
  for (const citation of citations) {
    ...
    const sourceLine = '[source id="' + escapeXmlAttr(citation.sourceId ?? "") + '" document-id="' + (Number.isSafeInteger(documentId) && documentId > 0 ? String(documentId) : "") + '" file="' + escapeXmlAttr(citation.originalName ?? "") + '" locator="' + escapeXmlAttr(citation.locator ?? "") + '"]\n';
...
async function loadCodexAccessToken({ codexAuth = "disabled", codexHome, env = process.env } = {}) {
  ...
  const file = join7(codexHome || env.CODEX_HOME || defaultCodexHome(), "auth.json");
  let raw;
  try {
...
async function readRc2Documents(dshHome) {
  const storageRoot = join8(dshHome, "storages");
  const workspaceFile = join8(storageRoot, "workspace.json");
  const projectionFile = join8(storageRoot, "session_projcache.json");
...
function resolveMaintenanceRoot({ dshHome }) {
  ...
  return join9(dshHome, "cyberpunk-workbench", "maintenance");
}
function createPurgeJobStore({ dshHome }) {
  const root = resolveMaintenanceRoot({ dshHome });
  const readyRoot = join9(root, "ready");
  const lockPath = join9(root, "active.lock.json");
  const jobDirectory = (jobId) => join9(root, assertSafeId(jobId, "job id"));
  const statePath = (jobId) => join9(jobDirectory(jobId), "state.json");
  const manifestPath = (jobId) => join9(jobDirectory(jobId), "manifest.json");
  const readyPath = (generation) => join9(readyRoot, `${assertSafeId(generation, "generation")}.json`);
...
import { basename, dirname as dirname3, isAbsolute as isAbsolute3, join as join10, relative as relative2, resolve as resolve4 } from "node:path";
...
async function assertRootPathSafe(ops, rootPath, anchorPath = dirname3(rootPath), { checkInternals = true, requireDirectories = checkInternals } = {}) {
  ...
  for (const current2 of [
    join10(rootPath, ".disabled"),
    join10(rootPath, ".staging"),
    join10(rootPath, ".transactions")
  ]) {
    const stat = requireDirectories ? await inspectPath(current2) : await optionalInspect(ops, current2, "lstat");
    if (stat?.isSymbolicLink()) {
      throw mutationError(SKILL_ERROR_CODES.PERMISSION_DENIED, "Skill root contains a symbolic link");
    }
    if (requireDirectories && stat && !stat.isDirectory()) {
...
async function copyTree(ops, source, destination) {
  ...
  for (const rawEntry of await readDirectory(ops, source)) {
    ...
    const from = join10(source, name);
    const to = join10(destination, name);
...
async function writeTransaction(ops, transactionsPath, descriptor) {
  const descriptorPath = join10(transactionsPath, `${descriptor.id}.json`);
  const temporaryPath = join10(transactionsPath, `.${descriptor.id}.json.tmp`);
...
function transactionPaths(rootPath, descriptor) {
  ...
  return {
    final: join10(rootPath, finalRelative),
    incoming: join10(rootPath, ".staging", id, "incoming"),
    previous: join10(rootPath, ".staging", id, "previous"),
    staging: join10(rootPath, ".staging", id)
  };
}
async function scanBundle({ rootPath, bundlePath, state, ops }) {
  const diagnostics = [];
  const skillPath = join10(bundlePath, "SKILL.md");
  let skillStat;
  try {
...
async function scanCatalogRoot({ rootPath, ops }) {
  ...
  for (const rawEntry of entries) {
    ...
    const path = join10(rootPath, name);
  ...
  const disabledPath = join10(rootPath, ".disabled");
  let disabledStat;
  try {
  ...
  if (disabledStat?.isSymbolicLink()) {
    ...
    for (const rawEntry of await readDirectory(ops, disabledPath)) {
      const name = entryName(rawEntry);
      if (typeof name !== "string" || name === "") continue;
      const path = join10(disabledPath, name);
      const stat = await inspect(ops, path, "lstat");
      if (stat.isSymbolicLink()) {
        diagnostics.push(diagnostic(path, "symlink", "Skill entries cannot be symbolic links"));
        continue;
...
async function recoverTransactions({ rootPath, ops, identity }) {
  ...
  const transactionsPath = join10(rootPath, ".transactions");
  ...
  for (const rawEntry of await readDirectory(ops, transactionsPath)) {
    ...
    const descriptorPath = join10(transactionsPath, name);
    ...
    const alternate = descriptor.state === "disabled" ? join10(rootPath, descriptor.name) : join10(rootPath, ".disabled", descriptor.name);
...
function createSkillManager({ dshHome, repos, fileOps = defaultFileOps, revealPath: injectedRevealPath } = {}) {
  ...
  async function readState(target, name) {
    const enabledPath = join10(target.rootPath, name);
    const disabledPath = join10(target.rootPath, ".disabled", name);
  ...
  async function importArchive({ scope, projectId, archiveBytes, sourceName, replace = false } = {}) {
    ...
    const temporaryRoot = await callFs("mkdtemp", ops.mkdtemp, join10(tmpdir(), ".cpwb-skill-import-"));
    let extractedRoot;
    try {
      const canonicalTempRoot = await callFs("realpath", ops.realpath, temporaryRoot);
      extractedRoot = join10(canonicalTempRoot, "incoming");
      ...
      return await serialize(key, async () => {
        ...
        const stagingRoot = join10(target.rootPath, ".staging", transactionId);
        const stagingIncoming = join10(stagingRoot, "incoming");
        await callFs("mkdir", ops.mkdir, stagingRoot, { recursive: true });
        await copyTree(ops, extractedRoot, stagingIncoming);
        const finalPath = existing?.state === "disabled" ? join10(target.rootPath, ".disabled", summary.name) : join10(target.rootPath, summary.name);
        const transactionsPath = join10(target.rootPath, ".transactions");
        if (!existing) {
          try {
          ...
          const previous = join10(stagingRoot, "previous");
          const descriptor = {
            version: 1,
            id: transactionId,
            name: summary.name,
  ...
  async function setEnabled({ scope, projectId, name, enabled } = {}) {
    ...
    return serialize(`${target.identity}\0${name}`, async () => {
      ...
      const destination = desired === "disabled" ? join10(target.rootPath, ".disabled", name) : join10(target.rootPath, name);
  ...
  ctx.effect(() => {
    ...
    try {
      ...
      const sessionWorkspace = async ({ kind, scopeId }) => {
        ...
        const path = kind === "knowledge_base" ? join11(dataDir, "knowledge-bases", String(scopeId)) : join11(dataDir, "sessions", "independent");
      ...
      const deleteContainer = async ({ kind, id, sessionPolicy }) => {
        ...
        try {
          ...
          for (const document of plan.orphanDocuments) {
            await unlink(join11(dataDir, "files", document.sha256)).catch((error) => {
              if (error?.code !== "ENOENT") throw error;
            });
          }
          return removed;
