...
import { dirname as dirname3, isAbsolute as isAbsolute3, join as join10, relative as relative2, resolve as resolve4 } from "node:path";
...
async function assertRootPathSafe(ops, rootPath, anchorPath = dirname3(rootPath), { checkInternals = true } = {}) {
  ...
  for (const current2 of [
    ...
    const stat = await optionalInspect(ops, current2, "lstat");
    if (stat?.isSymbolicLink()) {
      throw mutationError(SKILL_ERROR_CODES.PERMISSION_DENIED, "Skill root contains a symbolic link");
    }
  }
...
async function resolveTarget({ scope, projectId, dshHome, repos, ops }) {
  ...
  if (scope === "global") {
    ...
    return { scope: { kind: "global" }, rootPath: resolve4(dshHome, "skills"), anchorPath: resolve4(dshHome) };
  }
  ...
  return {
    ...
    rootPath: resolve4(projectPath, ".dsh", "skills"),
