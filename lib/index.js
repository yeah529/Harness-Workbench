...
function createRepositories(db) {
  ...
  const workbenchSessions = {
    upsert({ sessionId, scopeKind, scopeId, chatId = null, provider, model, reasoningEffort = null, now = /* @__PURE__ */ new Date() }) {
      const iso = nowIso(now);
      db.prepare(
        "INSERT INTO workbench_sessions (session_id, scope_kind, scope_id, chat_id, provider, model, reasoning_effort, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(session_id) DO UPDATE SET scope_kind = excluded.scope_kind, scope_id = excluded.scope_id, chat_id = excluded.chat_id, provider = excluded.provider, model = excluded.model, reasoning_effort = excluded.reasoning_effort, updated_at = excluded.updated_at"
    ...
    list({ scopeKind, scopeId, limit = 100, offset = 0 }) {
      ...
      return db.prepare(
        WORKBENCH_SESSION_SELECT + "WHERE ws.scope_kind = ? AND ws.scope_id = ? ORDER BY ws.updated_at DESC, ws.rowid DESC LIMIT ? OFFSET ?"
    ...
    latest({ scopeKind, scopeId }) {
      const row = db.prepare(
        WORKBENCH_SESSION_SELECT + "WHERE ws.scope_kind = ? AND ws.scope_id = ? ORDER BY ws.updated_at DESC, ws.rowid DESC LIMIT 1"
  ...
  const knowledgeChats = {
    create({ knowledgeBaseId, title = null, dshSessionId = null, now = /* @__PURE__ */ new Date() }) {
      const iso = nowIso(now);
      db.prepare(
    ...
    listActivityByProject(projectId) {
      return db.prepare(
        "SELECT kc.id AS chat_id, kc.knowledge_base_id, kc.dsh_session_id, kc.updated_at FROM knowledge_chats kc JOIN project_knowledge_bases pkb ON pkb.knowledge_base_id = kc.knowledge_base_id WHERE pkb.project_id = ? ORDER BY kc.id"
    ...
    bindSession({ id, dshSessionId, now = /* @__PURE__ */ new Date() }) {
      const iso = nowIso(now);
      const info = db.prepare(
...
function createApi({ repos, queue, ollama, retriever, dataDir, services = {}, sessions, settings, embeddingFactory, onEmbeddingConfigChange, credentials, codexAuth, dshAdapter, logger = console, networkEnv = process.env }) {
  ...
  async function handleChatSessionCreate(req, res) {
    ...
    const body = await readJsonBody(req);
    const projectId = body.projectId === void 0 ? void 0 : requirePositiveInt(body, "projectId");
    const knowledgeBaseId = body.knowledgeBaseId === void 0 ? void 0 : requirePositiveInt(body, "knowledgeBaseId");
    if (projectId !== void 0 && knowledgeBaseId !== void 0) {
      throw new ApiError(422, "INVALID_SCOPE", "provide at most one of projectId or knowledgeBaseId");
    }
    const title = optionalString(body, "title");
    const chatId = body.chatId === void 0 ? void 0 : requirePositiveInt(body, "chatId");
    const resumeSessionId = optionalString(body, "resumeSessionId");
    const input = { title };
    if (projectId !== void 0) input.projectId = projectId;
    if (knowledgeBaseId !== void 0) input.knowledgeBaseId = knowledgeBaseId;
    if (chatId !== void 0) input.chatId = chatId;
...
function createWorkbenchRagPreStep({ retriever, scope, onQuestion }) {
...
function createSessionService({ ctx, repos, retriever, sessionWorkspace }) {
  ...
  function registerHandle(sessionId, { dispose: dispose2, cleanup, scope, chatId, owned }) {
  ...
  async function resolveScope({ projectId, knowledgeBaseId }) {
    const hasProject = projectId !== void 0 && projectId !== null;
    const hasKb = knowledgeBaseId !== void 0 && knowledgeBaseId !== null;
    if (hasProject && hasKb) {
      throw new WorkbenchSessionError(
    ...
    if (hasProject) {
      ...
      return { workspaceId: project.workspaceId, cwd: void 0, scope: { kind: "project", scopeId: projectId } };
    }
    ...
    return { workspaceId: void 0, cwd: process.cwd(), scope: { kind: "knowledge_base", scopeId: knowledgeBaseId } };
  }
      ...
      const handle = await ctx.agents.resume({
        ...
        setup: async (agentCtx) => {
          ...
          if (typeof agentCtx.on === "function") {
            const dispose2 = agentCtx.on("agent/pre-step", createWorkbenchRagPreStep({
              retriever,
              scope,
              onQuestion: (question) => recordTitle(sessionId, question)
            }), { prepend: true });
            if (typeof dispose2 === "function") setupDisposers.push(dispose2);
          }
        }
      });
      let disposed = false;
  ...
  async function persistOwnedSession({ sessionId, scope, chatId = null, selection, dispose: dispose2, rollback }) {
    try {
  ...
  async function reopenScopedSession({ sessionId, scope }) {
    ...
    const { dispose: dispose2 } = await resumeWorkbenchSession(sessionId, scope);
    registerHandle(sessionId, { dispose: dispose2, scope, chatId: null, owned: true });
    repos.workbenchSessions.touch(sessionId);
    return { sessionId, scope, reused: true };
  }
  async function reopenKnowledgeChat({ knowledgeBaseId, chatId, workspaceId, cwd, scope }) {
    ...
    if (sessionId) {
      ...
      const saved = repos.workbenchSessions.get(sessionId);
      const { dispose: dispose2 } = await resumeWorkbenchSession(sessionId, scope);
      registerHandle(sessionId, { dispose: dispose2, scope, chatId, owned: true });
      if (saved) {
        repos.workbenchSessions.touch(sessionId);
      } else {
  ...
  async function createSession({ projectId, knowledgeBaseId, title = null, chatId, resumeSessionId, scheduled = false }) {
    ...
    const hasChatId = chatId !== void 0 && chatId !== null;
    if (resumeSessionId !== void 0 && resumeSessionId !== null) {
    ...
    if (!(scheduled && scope.kind === "project")) {
      await persistOwnedSession({
        ...
        selection: { provider: DEFAULT_PROVIDER, model: DEFAULT_MODEL },
        dispose: created.dispose,
        rollback: resultChatId === null ? void 0 : () => repos.knowledgeChats.remove?.(resultChatId)
      });
    }
    return {
      sessionId: created.sessionId,
  ...
  async function submitPrompt({ sessionId, question, projectId, knowledgeBaseId }) {
    ...
    if (projectId !== void 0 && (entry.scope.kind !== "project" || entry.scope.scopeId !== projectId)) {
      throw new WorkbenchSessionError(SESSION_ERROR_CODES.SCOPE_MISMATCH, "session scope does not match the declared project");
    }
    if (knowledgeBaseId !== void 0 && (entry.scope.kind !== "knowledge_base" || entry.scope.scopeId !== knowledgeBaseId)) {
...
// src/host/index.js
var inject = ["webServer", "agents", "sessions", "workspaceRegistry", "credentials"];
