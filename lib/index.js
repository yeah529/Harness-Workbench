...
function createIndexQueue({ repos, indexer }) {
  ...
  return {
    ...
    enqueue(task) {
      ...
      const entry = { task, resolve: resolve3, promise };
    ...
    idle() {
      ...
      return new Promise((resolve3) => idleWaiters.push(resolve3));
    },
    /**
...
function createContextResolver({ repos, sourceAccess } = {}) {
  ...
  function rebase({ sessionId, toScope }) {
    ...
    return resolve3({ sessionId });
  }
  function resolveForPrompt({ sessionId, oneShotSources = [] }) {
    const persistent = resolve3({ sessionId }).filter((source) => source.available);
  ...
  return { resolve: resolve3, validate, setOverride, removeOverride, rebase, resolveForPrompt };
}
...
function createApi({ repos, queue, ollama, retriever, dataDir, services = {}, sessions, settings, embeddingFactory, onEmbeddingConfigChange, credentials, codexAuth, dshAdapter, logger = console, networkEnv = process.env }) {
  ...
  async function handleDocumentContent(req, res, { params, url }) {
    ...
    let bytes;
    try {
      bytes = await readFile2(join5(dataDir, "files", document.sha256));
    } catch {
...
import { dirname, isAbsolute, join as join7, relative, resolve as resolve2 } from "node:path";
...
async function readRc2Documents(dshHome) {
  const storageRoot = join7(dshHome, "storages");
  const workspaceFile = join7(storageRoot, "workspace.json");
  const projectionFile = join7(storageRoot, "session_projcache.json");
...
async function probeRc2PurgeBackend({ dshHome, dataDir }) {
  try {
    ...
    const databasePath = resolve2(normalizedData, "workbench.sqlite");
...
function resolveMaintenanceRoot({ dshHome }) {
  ...
  return join8(dshHome, "cyberpunk-workbench", "maintenance");
}
function createPurgeJobStore({ dshHome }) {
  const root = resolveMaintenanceRoot({ dshHome });
  const readyRoot = join8(root, "ready");
  const lockPath = join8(root, "active.lock.json");
  const jobDirectory = (jobId) => join8(root, assertSafeId(jobId, "job id"));
  const statePath = (jobId) => join8(jobDirectory(jobId), "state.json");
  const manifestPath = (jobId) => join8(jobDirectory(jobId), "manifest.json");
  const readyPath = (generation) => join8(readyRoot, `${assertSafeId(generation, "generation")}.json`);
  ...
  return {
    ...
    async create(request) {
      ...
      await ensureRoot();
      await mkdir3(jobDirectory(jobId), { mode: 448 });
      const created = {
        ...cloneJson(request),
        jobId,
        state: "queued",
    ...
    async writeReady(generation, payload) {
      await mkdir3(readyRoot, { recursive: true, mode: 448 });
  ...
  ctx.effect(() => {
    ...
    try {
      ...
      const sessionWorkspace = async ({ kind, scopeId }) => {
        ...
        const path = kind === "knowledge_base" ? join9(dataDir, "knowledge-bases", String(scopeId)) : join9(dataDir, "sessions", "independent");
      ...
      const deleteContainer = async ({ kind, id, sessionPolicy }) => {
        ...
        try {
          ...
          for (const document of plan.orphanDocuments) {
            await unlink(join9(dataDir, "files", document.sha256)).catch((error) => {
