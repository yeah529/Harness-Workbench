...
function projectSessionActivity(repos, projectId, date, timeZone) {
  return (repos.knowledgeChats?.listActivityByProject?.(projectId) ?? []).filter((activity) => isLocalDate(activity.updatedAt, date, timeZone));
}
...
function createScheduler({ repos, clock = () => /* @__PURE__ */ new Date(), runPrompt, intervalMs = 6e4, timeZone = DEFAULT_TIME_ZONE } = {}) {
