...
function transactionPaths(rootPath, descriptor) {
  ...
  const finalRelative = state === "disabled" ? join10(".disabled", name) : name;
  return {
...
async function resolveTarget({ scope, projectId, dshHome, repos, ops }) {
  ...
  if (scope === "global") {
    ...
    return { scope: { kind: "global" }, rootPath: resolve4(dshHome, "skills") };
  }
      ...
      return await serialize(key, async () => {
        ...
        if (!existing) {
          ...
          try {
            ...
            await ops.rename(stagingIncoming, finalPath);
            movedIncoming = true;
            const installed2 = await scanBundle({ rootPath: target.rootPath, bundlePath: finalPath, state: existing.state, ops });
            if (!installed2.item || installed2.diagnostics.length > 0) {
