...
4. Host 先在系统临时目录中安全解压并识别唯一 Skill 根。
5. Host 解析 `SKILL.md`，校验身份、描述、目录结构和全部资源限制。校验失败时不创建项目 `.dsh/skills`。
6. 只有完整校验成功后，Host 才创建目标 Skill 根及其随机 `.staging` 事务目录，并将规范化内容复制到该临时目录。
